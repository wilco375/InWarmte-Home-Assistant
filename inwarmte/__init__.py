...
from .exceptions import InWarmteException
...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    """Set up InWarmte from a config entry."""
    # Create the InWarmte client
    inwarmte = InWarmte(
    ...
    async def async_update_data() -> dict[str, dict[str, Any]]:
        return await async_update_inwarmte(inwarmte)
...
async def async_unload_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    # If successful, unload the InWarmte client
    if unload_ok:
...
async def async_update_inwarmte(inwarmte: InWarmte) -> dict[str, dict[str, Any]]:
    """Update the data by performing a request to InWarmte."""
    try:
        # Note: asyncio.TimeoutError and aiohttp.ClientError are already
        # handled by the data update coordinator.
        async with asyncio.timeout(FETCH_TIMEOUT):
            if not inwarmte.is_authenticated():
                _LOGGER.warning("InWarmte is unauthenticated. Reauthenticating")
                await inwarmte.authenticate()

            current_measurements = await inwarmte.current_measurements()
...
def _get_cumulative_value(
    ...
    :param current_measurements: The result from the InWarmte client
    ...
    if source_type in current_measurements:
        if (
            ...
            return current_measurements[source_type][period_type]
    else:
        _LOGGER.error(
